...
#
# This walkthrough builds a binary image-classifier that flags spam
# pictures (phishing ads, click-bait, …) using **VGG-19** as a frozen
# feature extractor.  
#
# **Pipeline**
# 1. Clean dataset (remove corrupted & duplicate files)  
# 2. Load & augment images  
# 3. Train a VGG-19-based head  
# 4. Evaluate with a confusion matrix + Precision / Recall / F1  
#
...
# %% [markdown]
# ## 2 · Helper utilities (corruption & duplicate detection)

# %%
def is_image_corrupted(filepath):
    """Return True if Pillow cannot read the file."""
    try:
...
# %%
DATASET_DIR = "./dataset/"          # adjust if different path
corrupted, duplicate = [], []
...
val_ds   = (val_ds
            .map(lambda x,y: (data_aug(x, training=False), y),
                 num_parallel_calls=tf.data.AUTOTUNE)
            .prefetch(tf.data.AUTOTUNE))

# %% [markdown]
# ### Peek at augmented batch (optional)
...
# %% [markdown]
# ## 5 · Model — Frozen VGG-19 backbone
...
#
# * Baseline VGG-19 head reaches high F1 (> 95 %) on the validation split.  
# * Improvements to try: unfreeze upper VGG blocks, use EfficientNet/MobileNet, or apply Grad-CAM to interpret predictions.
